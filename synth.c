...
#endif
#define KEYS 20
const WORD keys[KEYS]={'Q','2','W','3','E','R','5','T','6','Y','7','U','I','9','O','0','P',219,187,221};
...
{
	const char *fxname[9]={"CHORUS","COMPRESSOR","DISTORTION","ECHO","FLANGER","GARGLE","I3DL2REVERB","PARAMEQ","REVERB"};
	HFX fx[9]={0}; // effect handles
	INPUT_RECORD keyin;
	DWORD r,buflen;
	printf("Simple Sinewave Synth\n"
			"--------------------------\n");
	...
	while (ReadConsoleInput(GetStdHandle(STD_INPUT_HANDLE),&keyin,1,&r)) 
	{
		...
		{
			if (keyin.Event.KeyEvent.wVirtualKeyCode==VK_SUBTRACT || keyin.Event.KeyEvent.wVirtualKeyCode==VK_ADD) 
			{
		...
		for (key=0;key<KEYS;key++)
			if (keyin.Event.KeyEvent.wVirtualKeyCode==keys[key]) 
			{
				if (keyin.Event.KeyEvent.bKeyDown && vol[key]<MAXVOL) 
				{
					...
					if (!keyin.Event.KeyEvent.bKeyDown && vol[key])
